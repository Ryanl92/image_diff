...
GIT_REMOTE='git@github.com:Ryanl92/image_diff.git'

SRC_DIR=${1}
TEMP_DIR=$(mktemp -d)
METHODS="near average bilinear cubic"

mkdir ${TEMP_DIR}/build

echo "Copying source files from ${SRC_DIR} to ${TEMP_DIR}"

find . -name '*.jp2' -execdir cp {} ${TEMP_DIR}/build/{} \;
...
pushd ${TEMP_DIR}
    ...
    git add ${TEMP_DIR}/script.sh
    git commit -m "Script used to generate the diffs"
